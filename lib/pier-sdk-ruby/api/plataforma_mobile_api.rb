...
module Pier
  class PlataformaMobileApi
    ...
    def listar_using_get26_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug "Calling API: PlataformaMobileApi.listar_using_get26 ..."
      end
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: PlataformaMobileApi#listar_using_get26\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
